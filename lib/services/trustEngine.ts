 ...
 *
 * Calculate a numeric Venue Health Score (0–100) and derive labels based on 8 Phase 5 factors:
 * Freshness, Coverage, Volatility, Verified status, Receipt count, Menu completeness, Crowd reports, Confidence.
 */
export async function calculateVenueHealth(venueId: string): Promise<{ score: number; label: string }> {
  try {
    const { data: venue } = await supabase
      .from('venues')
      .select('computed_confidence_score, last_price_updated_at, verified_venue, menu_completeness_score, predicted_price_variance_pct')
      .eq('id', venueId)
      .single();
    ...
    const { count: crowdReports } = await supabase
      .from('price_evidence')
      .select('id', { count: 'exact', head: true })
      .eq('venue_id', venueId)
    ...
    if (venue.last_price_updated_at) {
      ...
      freshnessScore = Math.max(0, 15 - (daysSince * 0.15));
    } else {
      freshnessScore = 0;
    }

    // 2. Confidence (0-20)
    const confScore = (Number(venue.computed_confidence_score || 0) / 100) * 20;

    // 3. Volatility penalty (-10 to 0)
    const volatility = Number(venue.predicted_price_variance_pct || 0);
    const volScore = Math.max(-10, -(volatility * 0.5));

    // 4. Verified Status (0 or 15)
    const verifiedScore = venue.verified_venue ? 15 : 0;

    // 5. Receipt Count (0-15)
    const recScore = Math.min(15, (receiptCount || 0) * 3);

    // 6. Menu Completeness (0-15)
    const menuScore = (Number(venue.menu_completeness_score || 0) / 100) * 15;

    // 7. Crowd Reports (0-10)
    const crowdScore = Math.min(10, (crowdReports || 0) * 2);

    // 8. Coverage/Parking (0-10)
    const coverageScore = 10; // Assuming basic location coverage is met if it exists

    let healthScore = freshnessScore + confScore + volScore + verifiedScore + recScore + menuScore + crowdScore + coverageScore;
...
/**
 * predictNextReverificationDate
 * Predicts exactly when a venue should be verified next using predictive heuristics.
 */
export function predictNextReverificationDate(
  confidence: number,
  volatilityVariance: number,
  lastVerifiedDate: Date,
  seasonalityMultiplier: number = 1.0
): Date {
  // Base intervals: High confidence = check in 90 days. Low confidence = check in 14 days.
  let baseDays = 90;
  if (confidence < 40) baseDays = 14;
  else if (confidence < 70) baseDays = 30;
  else if (confidence < 90) baseDays = 60;

  // Subtract days based on variance (highly volatile = check sooner)
  const variancePenalty = Math.min(baseDays * 0.8, volatilityVariance * 2); 
  
  const finalDays = Math.max(7, Math.round((baseDays - variancePenalty) * seasonalityMultiplier));
  
  const nextDate = new Date(lastVerifiedDate.getTime());
