...
export default function AreaLayout({ slug, areaName, description, spotsCount, children }: AreaLayoutProps) {
  ...
  if (template === "premium-void") {
    return (
      <div className="w-full bg-white text-text-primary">
        <div className="min-h-[40vh] flex flex-col justify-end px-6 pb-12 pt-24 border-b border-black/10">
          <h1 className="font-sans font-black text-5xl tracking-[-0.04em] uppercase leading-none mb-4">{areaName}</h1>
          <p className="type-body text-text-secondary text-md font-medium">{description}</p>
  ...
  if (template === "linear-strip") {
    return (
      <div className="w-full bg-white text-text-primary">
        <div className="py-12 px-6 border-b-2 border-black">
          <h1 className="font-sans font-black text-4xl tracking-tight uppercase leading-none">{areaName}</h1>
          <p className="type-body text-text-muted mt-2 text-sm">{description}</p>
        ...
        <DossierDropWrapper className="px-6 py-12 space-y-12 dossier-grid">
          {children}
        </DossierDropWrapper>
      </div>
    );
      ...
      <div className="w-full bg-white text-text-primary">
        <div className="p-6 border-b border-black">
          <div className="flex flex-col gap-4">
            <div>
              <h1 className="font-sans font-black text-3xl tracking-tight uppercase leading-none">{areaName}</h1>
              <p className="type-caption text-text-muted mt-1.5">{description}</p>
            </div>
            <span className="bg-black text-white text-[9px] font-mono uppercase tracking-widest px-3 py-1.5 rounded-[4px] self-start font-black">
        ...
        <div className="w-full">
          <DossierDropWrapper className="grid grid-cols-1 border-b border-black divide-y divide-black/15 dossier-grid">
            {children}
          </DossierDropWrapper>
        </div>
      </div>
  ...
  return (
    <div className="w-full bg-white text-text-primary">
      ...
      <div className="px-6 py-12">
        <DossierDropWrapper className="grid grid-cols-1 gap-8 dossier-grid">
          {children}
        </DossierDropWrapper>
      </div>
    </div>
