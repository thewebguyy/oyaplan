...
export default function EditorialGuideCard({ guide }: { guide: PlanningGuide }) {
  return (
    <div className="bg-white-sand border border-border-default/60 rounded-[32px] overflow-hidden p-6 sm:p-8 flex flex-col justify-between card-lift shadow-lagoon hover:shadow-lift-lagoon h-full relative group">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-border-default/30 group-hover:bg-lasgidi-yellow transition-colors" style={{ transitionDuration: 'var(--duration-lift)' }} />
      ...
      <div className="pt-6 mt-6 border-t border-border-default/40 flex items-center justify-between">
        <Link href={`/guides/${guide.slug}`}>
          <button className="type-label text-midnight-lagoon hover:underline decoration-lasgidi-yellow underline-offset-4 flex items-center gap-2 tap-feedback transition-[colors,text-decoration]" style={{ transitionDuration: 'var(--duration-hover)' }}>
            Read Guide
            <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" style={{ transitionDuration: 'var(--duration-hover)' }} />
          </button>
        </Link>
      </div>
    </div>
